...
(deftest setFont
  (eq?
   [{:title         "Graphics setFont test doc"
    ...
    [:heading "setFont"]
    [:paragraph
     "Available fonts vary greatly among operating system and individual installations.  The graphics-2d font capability differs from the rest of clj-pdf, relying on an underlying Java AWT system.  Currently, that system imports fonts only from common system directories (TrueType, OpenType, and Type1 fonts). This makes choosing a single cross-platform test font that isn't in the default set (SansSerif(Helvetica), Serif(Times), Monospaced(Courier)) unlikely."]
    [:paragraph
     "To be reasonably cross-platform, these tests work generally under at least one standard operating system font, to the extent that exists at all in a given OS family.  This has been verified on at least one system of the following: MacOS 10.12.5."]
    ...
    [:graphics {:under false :translate [30 530]}
     (fn [g2d]
    ...
    [:graphics {:under false :translate [30 650]}
     (fn [g2d]
