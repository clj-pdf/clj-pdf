	...
	public void startElement(String tag, HashMap h) {
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				cljpdf.text.List list = new cljpdf.text.List(false);
				try{
			...
			if (tag.equals(HtmlTags.ORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				cljpdf.text.List list = new cljpdf.text.List(true);
				try{
					list.setIndentationLeft(new Float(cprops.getProperty("indent")).floatValue());
				}catch (Exception e) {
					list.setAutoindent(true);
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)
				...
				if (stack.empty())
					return;
				Object obj = stack.pop();
				if (!(obj instanceof cljpdf.text.List)) {
					stack.push(obj);
					return;
				}
				if (stack.empty())
			...
			if (tag.equals(HtmlTags.LISTITEM)) {
				...
				if (!(list instanceof cljpdf.text.List)) {
					stack.push(list);
					return;
				}
				ListItem item = (ListItem) obj;
				((cljpdf.text.List) list).add(item);
