...
*/
    ...
    private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
        ...
        try {
            cljpdf.text.Image image = null;
            if(!convertImagesToJPEG){
                image = cljpdf.text.Image.getInstance(img, bgColor);
            }
            else{
                ...
                scaled.flush();
                scaled = null;
                image = cljpdf.text.Image.getInstance(baos.toByteArray());

            }
            if (mask!=null) {
                cljpdf.text.Image msk = cljpdf.text.Image.getInstance(mask, null, true);
                msk.makeMask();
    ...
    private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
        if (paint instanceof Color) {
            ...
            try {
                ...
                cljpdf.text.Image image = cljpdf.text.Image.getInstance(img, null);
